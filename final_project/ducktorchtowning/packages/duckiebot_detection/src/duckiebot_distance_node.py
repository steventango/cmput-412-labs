...
class DuckiebotDistanceNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        super(DuckiebotDistanceNode, self).__init__(
        ...
        self.sub_info = rospy.Subscriber(
            "/{}/camera_node/camera_info".format(self.host),
            CameraInfo,
        ...
        self.pub_distance_to_robot_ahead = rospy.Publisher(
            "/{}/duckiebot_distance_node/distance".format(self.host),
            Float32,
            queue_size=1,
        )
    ...
    def cb_process_centers(self, vehicle_centers_msg):
        ...
        if detection:
            self.calc_circle_pattern(
                vehicle_centers_msg.H, vehicle_centers_msg.W
            )
            points = np.zeros(
            ...
            if success:
                ...
                mean_reproj_error = np.mean(
                    np.sqrt(
                        np.sum(
                            (np.squeeze(points_reproj) - points) ** 2, axis=1
                        )
                    )
                )
    ...
    def calc_circle_pattern(self, height, width):
        ...
        if (
            self.last_calc_circle_pattern is None
            or self.last_calc_circle_pattern != (height, width)
        ):
